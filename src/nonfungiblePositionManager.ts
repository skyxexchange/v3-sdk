...
export abstract class NonfungiblePositionManager {
  public static FACTORY_INTERFACE: Interface = new Interface(ISkyXV3Factory.abi)
  ...
  private static encodeCreate(pool: Pool): string {
    return NonfungiblePositionManager.FACTORY_INTERFACE.encodeFunctionData('createPool', [
      pool.token0.address,
      pool.token1.address,
      pool.fee,
    ])
  ...
  private static encodeInitialize(pool: Pool): string {
    return NonfungiblePositionManager.INTERFACE.encodeFunctionData('createAndInitializePoolIfNecessary', [
      pool.token0.address,
      pool.token1.address,
      pool.fee,
  ...
  public static initializeCallParameters(pool: Pool): MethodParameters {
    return {
  ...
  public static addCallParameters1(position: Position, options: AddLiquidityOptions): MethodParameters {
    invariant(JSBI.greaterThan(position.liquidity, ZERO), 'ZERO_LIQUIDITY')
  ...
  public static addCallParameters2(position: Position, options: AddLiquidityOptions): MethodParameters {
    invariant(JSBI.greaterThan(position.liquidity, ZERO), 'ZERO_LIQUIDITY')
